...
run_event_study <- function(data, min_bin = -10, max_bin = 10, 
  ...
  # Create binned relative time variable name
  binned_var_name <- paste0("rel_time_binned_", abs(min_bin), "_", abs(max_bin))
  
  # Create binned relative time variable in the copy
  data_copy <- data_copy %>%
    mutate(
...
# Using the run_event_study function with binning at ±10
...
# Using the run_event_study function with binning at ±11
...
Note here we use $\Sigma_0 \cap \Sigma_1$ (post treatment, and pre treatment) so that we can also see the pre-treatment effects. 
We discard observations with that

```{r imputation-event-study}
# Keep only treated units and add binned relative time
...
1. **Pre-treatment effects**: Pre treatment coefficients are not different from zero, supporting parallel trends
2. **Post-treatment effects**: A similar post-treatment effect pattern as TWFE, but potentially more reliable due to avoiding forbidden comparisons
3. **Comparison to TWFE**: There is no siginificant difference in the pattern when compared to TWFE, suggesting limited bias from forbidden comparisons in this case. As we might have quessed from the Bacon decomposition.
...
For manua ATT estimation, we take the simple average of the imputed treatment effects (tau_hat) for treated observations only (i.e. those with post == 1).
...
#   Value = c(
#     round(abs(att_manual$ATT - att_dimp), 4),
#     round(att_manual$SE_incorrect / se_dimp, 2)
#   )
# ) %>%
